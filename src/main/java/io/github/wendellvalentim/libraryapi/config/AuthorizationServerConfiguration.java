...
public class AuthorizationServerConfiguration {
    ...
    @Bean
    @Order(1)
    public SecurityFilterChain oathServerSecurityFilterChain(HttpSecurity http) throws Exception{

        OAuth2AuthorizationServerConfiguration.applyDefaultSecurity(http);


        http.getConfigurer(OAuth2AuthorizationServerConfigurer.class)
                //permita com que o token em mãos consigo as informações de quem gerou, usuario e afins.
    ...
    public AuthorizationServerSettings authorizationServerSettings() {
        return AuthorizationServerSettings.builder()
                ...
                // oter a chave pulica para verificar a assinatura do token
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            ...
            if(principal instanceof CustomAuthentication authentication) {
                OAuth2TokenType tipoTokken = context.getTokenType();
                if(OAuth2TokenType.ACCESS_TOKEN.equals(tipoTokken)) {
